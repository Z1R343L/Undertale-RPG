...
class duel:
    ...
    async def end(self):
        if str(self.author.id) not in self.bot.duels:
            return
        del self.bot.duels[str(self.author.id)]
...
class Fight(commands.Cog):
    ...
    async def food(self, inter: disnake.MessageInteraction, item: str, uid: int) -> None:
        ...
        msg_id = inter.bot.duels[str(uid)].menus[0]
        inter.bot.duels[str(uid)].menus.remove(msg_id)
        ...
        if item == "back":
            ...
            return await inter.bot.duels[str(uid)].menu()

        await inter.edit_original_message(components=[])

        return await getattr(inter.bot.duels[str(uid)], inter.bot.items[item]["func"])(item)
    ...
    async def action(self, inter: disnake.MessageInteraction, action: str, uid: int) -> None:
        ...
        return await getattr(inter.bot.duels[str(uid)], action)()
    ...
    async def duel(self, inter):
        ...
        if str(inter.author.id) in inter.bot.duels:
        ...
        fight = duel(inter.author, inter.bot, monster, inter, 0, inter.channel)
        fight.bot.duels[str(inter.author.id)] = fight
        try:
            await fight.menu()
        except Exception as e:
